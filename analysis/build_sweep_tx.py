...
def build(n, dest, fee_rate, take_all=False, wif=None, utxos=None,
    ...
    if fmt == 'ps1':
        ...
        A("# 0 false = locktime, replaceable. RBF is OFF on purpose: a replaceable")
        A("# transaction can be evicted by a higher fee, and this one exposes the")
        A("# public key as soon as it is seen. Never rely on the RPC default here.")
        A("$raw = & bitcoin-cli createrawtransaction $inputs $outputs 0 false")
        A("$raw")
        A("")
    ...
    A("")
    A("# --- 1. build the unsigned transaction ---------------------------------")
    A("# The trailing  0 false  is locktime and replaceable. RBF is switched OFF")
    A("# deliberately: a replaceable transaction can be evicted by anyone paying")
    A("# a higher fee, and this one publishes the public key the moment it is")
    A("# seen. Do not rely on the RPC default -- state it.")
    A("RAW=$(bitcoin-cli createrawtransaction \\")
    A("  '%s' \\" % json.dumps(inputs, separators=(',', ':')))
    A("  '%s' 0 false)" % json.dumps(outputs, separators=(',', ':')))
    A("")
    A("echo \"$RAW\"          # this is the unsigned transaction")
    A("")
    A("# --- 2. READ IT BACK before signing ------------------------------------")
    A("# Confirm the destination and the amount with your own eyes. The fee is")
    A("# the difference between the inputs and this output -- Core never shows")
    ...
    A("SIGNED=$(bitcoin-cli signrawtransactionwithkey \"$RAW\" \\")
    A("  '[\"%s\"]' \\" % (wif or "<WIF_FROM_FOUND_KEY.txt>"))
    A("  '%s')" % json.dumps(prevtxs, separators=(',', ':')))
    A("")
    A("echo \"$SIGNED\"       # check that it reads  \"complete\": true")
    A("")
    A("# Extract just the signed hex to carry out (needs jq; otherwise copy the")
    A("# \"hex\" field from the output above by hand):")
    A("#   echo \"$SIGNED\" | jq -r .hex > signed_tx.txt")
    A("")
    A("# --- 4. broadcast (ONLINE machine) -------------------------------------")
    A("# The signed hex is safe to carry out; it no longer contains the key,")
    A("# but it DOES expose the public key the moment it is seen. Prefer a")
